...
jQuery(document).ready(function($) {
    ...
    if (wooco_vars.selector == 'ddslick') {
        $('.wooco_component_product_select').each(function() {
            ...
            if (_this.val() != '') {
                var _selected_default = $("option:selected", this);
        ...
        $('.wooco_component_product_select').on('select2:select', function(e) {
            var _this = $(this);
            var _this_selection = _this.closest('.wooco_component_product_selection');
            var _this_component = _this.closest('.wooco_component_product');
            var _selected = $(e.params.data.element);
            wooco_selected(_selected, _this_selection, _this_component);
            wooco_check_ready();
        });
    } else {
    ...
    function wooco_select2_state(state) {
        ...
        if ($(state.element).attr('data-imagesrc') != '') {
            $state = $('<span class="image"><img src="' + $(state.element).attr('data-imagesrc') + '"/></span><span class="info"><span>' + state.text + '</span> <span>' + $(state.element).attr('data-description') + '</span></span>');
        } else {
            $state = $('<span class="info"><span>' + state.text + '</span> <span>' + $(state.element).attr('data-description') + '</span></span>');
        }
        return $state;
    }
    ...
    function wooco_check_ready() {
        ...
        $wooco_components.find('.wooco_component_product').each(function() {
            ...
            if ((_this.attr('data-optional') == 'no') && (_this.attr('data-id') <= 0)) {
                is_selection = true;
            }
        });
        if (is_selection) {
    ...
    function wooco_calc_price() {
        var total = 0;
        if (($wooco_components.attr('data-pricing') == 'only') && ($wooco_components.attr('data-price') > 0)) {
            total = Number($wooco_components.attr('data-price'));
        } else {
            $wooco_components.find('.wooco_component_product').each(function() {
                var _this = $(this);
                if ((_this.attr('data-price') > 0) && (_this.attr('data-qty') > 0)) {
                    total += Number(_this.attr('data-price')) * Number(_this.attr('data-qty'));
                }
            });
            if (($wooco_components.attr('data-percent') > 0) && ($wooco_components.attr('data-percent') < 100)) {
                total = total * (100 - Number($wooco_components.attr('data-percent'))) / 100;
            }
        ...
        total_html += '</span>';
        if (($wooco_components.attr('data-pricing') != 'only') && (parseFloat($wooco_components.attr('data-percent')) > 0) && (parseFloat($wooco_components.attr('data-percent')) < 100)) {
        ...
        if (wooco_vars.change_price != 'no') {
            ...
            if ((wooco_vars.price_selector != null) && (wooco_vars.price_selector != '')) {
